...
<script>
  ...
  function openTemplate(id){
	      ...
	      <div class="modal fade" id="newDocument" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true" class="ti-close"></span></button>
					<h4 class="modal-title" id="myModalLabel1"><input class="form-control" value="Empty Document"/></h4>
				</div>
				<div class="modal-body" align="center">
				  <div class="row">
            <h4> (1) Select a Template</h4>
            <?php 
                $query=mysql_query("SELECT * FROM document_template");
                while($row=mysql_fetch_array($query)){
                  echo "<div class='col-md-4' align='center' style='cursor:pointer'> <b>$row[name]</b> <br><a href='pdf.php?id=$row[id]' target='_blank'> <img src='$row[picture]' width='50%'> <br> </div>";
                }
            ?>
            <div align="right"><a href='javascript:void(0);'><small>Or create new empty template</small></a></div>
          </div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">CLOSE</button>
		...
		<div class="row">
			<div class="col-md-12 nopad-right">
              ...
              <div class="col-md-2">
                  ...
                  <hr/>
                  <center>
                    <h2><?php echo " <span class='ion-document-text'></span> &nbsp;$name"; ?></h2>
                  </center>
									<div class="profile-sidebar" >
									             <center>
											<button class="btn btn-success" onclick="openModal($('#newDocument'));">New Document</button>
                      <button class="btn btn-success">Templates</button>
                      <button class="btn btn-success">Signatures</button>
                      </center>
                    ...
                    <div class="table-responsive">
                      <table class="table">
                          ...
                          <?php 
                            $query=mysql_query("SELECT * FROM documents WHERE id_company=$idBusiness") or die(mysql_error());
                            $num=mysql_num_rows($query);
                           
                            if($num){
                              echo "<tr><th>ID</th><th>Name Document</th><th>Type</th><th>Action</th></tr>";
                              while($row=mysql_fetch_array($query)){
                              echo "<tr><th>$row[id]</th><th>$row[name]</th><th>$row[id_template]</th><th><button class='btn btn-danger'> View</button></th></tr>";
                            }
                            }else{
                             echo "<tr><td align='center'><h1>Not file founded</h1></td></tr>";
                            }
